  ...
  await Promise.all((data.folders||[]).map(async (f,i)=>{
    try{
      ...
      f.items=remote.map((x,i)=>{
        ...
        return prev || {src:x.download_url,type:pdf?'pdf':video?'video':'image',title:x.name.replace(/\.[^.]+$/,''),desc:'',id:`gh-${kind}-${i}`};
      });
...
function openFolder(id){
  ...
  const media=items.map((x,i)=>`<article class="viewer-item">
    <div class="viewer-media">${x.type==='video'
      ...
      : x.type==='pdf'
        ? `<div class="viewer-pdf"><iframe src="${esc(x._src||x.src)}" title="${esc(x.title||'PDF')}"></iframe><a href="${esc(x._src||x.src)}" target="_blank" rel="noopener">OPEN PDF ↗</a></div>`
