...
// Resolve a discipline from its visible name first. This prevents old/localStorage
// IDs from making one discipline display another discipline's 3D environment.
function disciplineKind(f,i){
  const n=String(f?.name||'').toLowerCase().replace(/\s+/g,' ').trim();
  if(n.includes('film')) return 'filmmaking';
  ...
  if(n.includes('3d')) return 'art3d';
  if(n.includes('logo')) return 'logo';
  if(n.includes('motion') || n.includes('visual')) return 'motion';
  return ['filmmaking','editing','graphic','color','photography','art3d','logo','motion'][i] || 'filmmaking';
}
function mediaFolderKey(f,i){ return disciplineKind(f,i); }
...
async function loadGitHubMedia(){
  ...
  await Promise.all((data.folders||[]).map(async (f,i)=>{
    try{
      const kind=mediaFolderKey(f,i);
      const r=await fetch(`https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/contents/media/${encodeURIComponent(kind)}`);
      if(!r.ok)return;
      const list=await r.json();
      if(!Array.isArray(list))return;
      const remote=list.filter(x=>x.type==='file' && MEDIA_EXT.test(x.name));
...
function pdfViewer(src, cls='pdf-viewer'){
  const id='pdf-'+Math.random().toString(36).slice(2);
  const el=`<div class="${cls}" data-pdf-viewer="${id}" data-pdf-src="${esc(src)}"><div class="pdf-loading">Loading PDF…</div><div class="pdf-pages"></div></div>`;
  requestAnimationFrame(()=>renderPDF(id,src));
  return el;
}
async function renderPDF(id,src){
  const host=document.querySelector(`[data-pdf-viewer="${id}"]`);
  if(!host || !window.pdfjsLib)return;
  const pages=host.querySelector('.pdf-pages');
  try{
    const task=window.pdfjsLib.getDocument({url:src,disableAutoFetch:false,disableStream:false});
    const pdf=await task.promise;
    host.querySelector('.pdf-loading')?.remove();
    const maxPages=pdf.numPages;
    for(let n=1;n<=maxPages;n++){
      const page=await pdf.getPage(n);
      const base=page.getViewport({scale:1});
      const width=Math.min(host.clientWidth-24,1400);
      const scale=Math.max(.55,Math.min(2.2,width/base.width));
...
function openFolder(id){
  ...
  const media=items.map((x,i)=>`<article class="viewer-item">
    <div class="viewer-media">${x.type==='video'
      ...
      : x.type==='pdf'
        ? pdfViewer(esc(x._src||x.src),'viewer-pdf')
...
document.addEventListener('keydown',e=>{if(e.key==='Escape')closeFolder(); if((e.ctrlKey||e.metaKey)&&['s','p'].includes(e.key.toLowerCase())){e.preventDefault();e.stopPropagation();}});
document.addEventListener('contextmenu',e=>{if(e.target.closest('.pdf-preview,.viewer-pdf'))e.preventDefault();});
