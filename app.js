  ...
  await Promise.all((data.folders||[]).map(async (f,i)=>{
    try{
      ...
      f.items=remote.map((x,i)=>{
        ...
        return prev || {src:x.download_url,type:video?'video':'image',title:x.name.replace(/\.[^.]+$/,''),desc:'',id:`gh-${kind}-${i}`};
      });
